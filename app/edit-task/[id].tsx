...
const Edit = () => {
	...
	const fetchTaskById = useTasksStore((state) => state.fetchTaskById);
	const updateTask = useTasksStore((state) => state.updateTask);

	useEffect(() => {
		const fetchTask = async () => {
			setLoading(true);
			const fetchedTask = await fetchTaskById(db, id);
			if (fetchedTask) {
				setTask({
					title: fetchedTask.title || "",
					description: fetchedTask.description || "",
					completed: fetchedTask.completed || false,
					tags: fetchedTask.tags ? fetchedTask.tags.split(",") : [],
				});
				setTags(fetchedTask.tags ? fetchedTask.tags.split(",") : []);
			}
			setLoading(false);
		};
	...
	const handleUpdateTask = async () => {
		try {
			await updateTask(db, id, { ...task, tags: tags.join(",") });
